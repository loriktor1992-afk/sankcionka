...
export default function SearchPage() {
  ...
  useEffect(() => {
    ...
    // Фильтр по топливу
    if (fuel) {
      filtered = filtered.filter(c => c.fuel?.toLowerCase().includes(fuel));
    }

    // Фильтр по приводу
    if (drive) {
      filtered = filtered.filter(c => c.drive?.toLowerCase().includes(drive));
    }

    // Фильтр по трансмиссии
    if (transmission) {
      filtered = filtered.filter(c => c.transmission?.toLowerCase().includes(transmission));
    }
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 py-10">
      <div className="container mx-auto px-4">
        ...
        {loading ? (
          <p className="text-lg text-gray-600 mb-8">Загрузка...</p>
        ) : (
          <p className="text-lg text-gray-600 mb-8">
            Найдено <strong className="text-blue-600">{results.length}</strong> автомобилей
          </p>
        )}
